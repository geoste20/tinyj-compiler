...
public final class ParserAndTranslator {
  ...
  private static void argumentList(MethodRec m) throws SourceFileErrorException
  {
     ...
     // check # of arguments against method record
     if (m.getArgCount() == MethodRec.NOT_KNOWN) {
       m.setArgCount(argCount);
     }
     else if (m.getArgCount() != argCount) {
       throw new SourceFileErrorException(
           "Method " + m.name + " called with wrong number of arguments"
       );
     }
  ...
  private static void whileStmt() throws SourceFileErrorException
  {
    ...
    int startAddr = Instruction.getNextCodeAddress();   // label a
    ...
    JUMPONFALSEinstr jInstr =
        new JUMPONFALSEinstr(Instruction.OPERAND_NOT_YET_KNOWN);   // JUMPONFALSE b

    statement();                        // body

    new JUMPinstr(startAddr);           // JUMP a

    jInstr.fixUpOperand(Instruction.getNextCodeAddress()); // b = here
  ...
  private static void expr4() throws SourceFileErrorException
  {
    ...
    if (   getCurrentToken() == GT
      ...
      switch (op) {
        case GT: new GTinstr(); break;
        case LT: new LTinstr(); break;
        case GE: new GEinstr(); break;
        case LE: new LEinstr(); break;
      }
    }
